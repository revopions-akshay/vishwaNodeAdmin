...
const CategoryList = () => {
  ...
  const deleteCategory = async (id) => {
    console.warn(id); 
    let result = await fetch(`http://localhost:5000/category/delete/${id}`,{
      method: "Delete"
    });
    result = await result.json();
    if(result){
                ...
                <CTableBody>
                  {category ? category.map((item,index) => (
                    <CTableRow v-for="item in tableItems" key={index}>
                      ...
                      <CTableDataCell className="text-center">
                      <div>{item.sub_category_count}</div>
                      </CTableDataCell>
                      <CTableDataCell className="text-center">
                      ...
                      <CTableDataCell>
                        <button type="button" onClick={()=>deleteCategory(item._id)}>Delete</button>
                        <a href={"/category/update/"+item._id}>Edit</a>
                      </CTableDataCell>
                    </CTableRow>
                  )) : <CTableRow></CTableRow>} 
                </CTableBody>
